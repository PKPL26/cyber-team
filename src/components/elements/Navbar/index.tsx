...
export default function Navbar() {
  ...
  // 1. Hydration fix: Ensure the component knows it has mounted on the client
  useEffect(() => {
    setMounted(true);
  }, []);
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              href={link.href}
              key={link.href}
              className={` transition-colors ${isActive && mounted ? "border-b-2 text-foreground" : "hover:text-foreground  hover:border-b-2"}`}
            >
              {link.label}
            </Link>
          );
